...
public class ValidationUtil {
    ...
    private static final String INSURANCE_POLICY_ID_REGEX = "^[A-Za-z0-9\\-]{3,30}$";
    ...
    /**
     * Validates date of birth: must be non-null, in the past, and patient must be at least 1 year old.
     */
    public static boolean isValidDateOfBirth(LocalDate dob) {
        if (dob == null) return false;
        LocalDate today = LocalDate.now();
        return dob.isBefore(today) && dob.plusYears(1).isBefore(today.plusDays(1));
    }

    /**
     * Validates insurance policy ID: alphanumeric with hyphens, 3-30 characters.
     */
    public static boolean isValidInsurancePolicyId(String policyId) {
        return policyId != null && Pattern.matches(INSURANCE_POLICY_ID_REGEX, policyId.trim());
    }
}
